...
if [ $# -lt 1 ] || [ $# -lt 2 ] 
then
	echo "Pour de l'aide faites la commande suivante: bash [nom_script] [nom_fichier] -h"
	exit 1	
fi
...
then
	...
	#Si aucune des options entrée n'est valide le programme ne s'éxecute pas
	if [ $help -eq 0 ] && [ $trid1 -eq 0 ] && [ $trid2 -eq 0 ] && [ $triL -eq 0 ] && [ $triT -eq 0 ] && [ $triS -eq 0 ]
	then
    	echo "La ou les options entrée ne sont pas valide."
		echo "Pour de l'aide faites la commande suivante: bash [nom_script] [nom_fichier] -h"
    	exit 1
	fi
